...
function [paraMap,resnorm,stats_roi] = hyModelTV(dwi,b,limit,initial,roi,stats,tvIter,alpha,const)
%-----------------------------------------------------------------------------------------------
...
parfor smoothn=1:tvIter % TV iteration
    ...
    for slice=1:vox
        ...
        [HYtvDDpfk, resnormHYtv] = lsqcurvefit(@allivimdki,D0_hy2,b,ydata_13b,limit(1,:),limit(2,:),options);
...
if stats==1
    ...
    meanROI_Dmap=mean(DmapHYtv_allb(roi));
    meanROI_Dpmap=mean(DpmapHYtv_allb(roi));
    meanROI_fmap=mean(fmapHYtv_allb(roi));
    meanROI_kmap=mean(kmapHYtv_allb(roi));
    % Calculating standard deviation of ROI given
    stdROI_Dmap=std(DmapHYtv_allb(roi));
    stdROI_Dpmap=std(DpmapHYtv_allb(roi));
    stdROI_fmap=std(fmapHYtv_allb(roi));
    stdROI_kmap=std(kmapHYtv_allb(roi));
